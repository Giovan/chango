...
type ChangoLogger struct {
	log15.Logger
}

// Set the systems default logger
// Default logs will be captured and handled by chango at level info
func SetDefaultLog(fromLog MultiLogger) {
	log.SetOutput(loggerRewrite{Logger: fromLog, Level: log15.LvlInfo, hideDeprecated: true})
	// No need to show date and time, that will be logged with Chango
	log.SetFlags(0)
}

func (rl *ChangoLogger) Debugf(msg string, param ...interface{}) {
	rl.Debug(fmt.Sprintf(msg, param...))
}

// Print a formatted info message
func (rl *ChangoLogger) Infof(msg string, param ...interface{}) {
	rl.Info(fmt.Sprintf(msg, param...))
}

// Print a formatted warn message
func (rl *ChangoLogger) Warnf(msg string, param ...interface{}) {
	rl.Warn(fmt.Sprintf(msg, param...))
}

// Print a formatted error message
func (rl *ChangoLogger) Errorf(msg string, param ...interface{}) {
	rl.Error(fmt.Sprintf(msg, param...))
}

// Print a formatted critical message
func (rl *ChangoLogger) Critf(msg string, param ...interface{}) {
	rl.Crit(fmt.Sprintf(msg, param...))
}

// Print a formatted fatal message
func (rl *ChangoLogger) Fatalf(msg string, param ...interface{}) {
	rl.Fatal(fmt.Sprintf(msg, param...))
}

// Print a formatted panic message
func (rl *ChangoLogger) Panicf(msg string, param ...interface{}) {
	rl.Panic(fmt.Sprintf(msg, param...))
}

// Print a critical message and call os.Exit(1)
func (rl *ChangoLogger) Fatal(msg string, ctx ...interface{}) {
	rl.Crit(msg, ctx...)
	os.Exit(1)
}

// Print a critical message and panic
func (rl *ChangoLogger) Panic(msg string, ctx ...interface{}) {
	rl.Crit(msg, ctx...)
	panic(msg)
}

// Override log15 method
func (rl *ChangoLogger) New(ctx ...interface{}) MultiLogger {
	old := &ChangoLogger{Logger: rl.Logger.New(ctx...)}
	return old
}

// Set the stack level to check for the caller
func (rl *ChangoLogger) SetStackDepth(amount int) MultiLogger {
...
func New(ctx ...interface{}) MultiLogger {
	r := &ChangoLogger{Logger: log15.New(ctx...)}
	//r.SetStackDepth(2)
	return r
}

// Set the handler in the Logger
func (rl *ChangoLogger) SetHandler(h LogHandler) {
