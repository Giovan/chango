...
// chango.ERROR, so that the developer does not need to check the return value to
...
var (
	...
	ErrCacheMiss    = errors.New("chango/cache: key not found")
	ErrNotStored    = errors.New("chango/cache: not stored")
	ErrInvalidValue = errors.New("chango/cache: invalid value")
)
