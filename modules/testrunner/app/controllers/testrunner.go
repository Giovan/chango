...
import (
	...
	"github.com/giovan/chango/testing"
)
...
// This structure is required by chango test cmd.
...
func (c TestRunner) Index() chango.Result {
...
func (c TestRunner) Suite(suite string) chango.Result {
...
func (c TestRunner) Run(suite, test string) chango.Result {
	...
	func() {
		...
		defer func() {
			if err := recover(); err != nil {
				...
				panicErr := chango.NewErrorFromPanic(err)
				if panicErr == nil {
					return
				}
				...
				tmpl, _ := chango.MainTemplateLoader.TemplateLang("TestRunner/FailureDetail.html", "")
...
// It is used by chango test command line tool.
func (c TestRunner) List() chango.Result {
...
func errorSummary(err *chango.Error) (message string) {
...
// formatResponse gets *chango.TestSuite as input parameter and
...
func init() {
	...
	// provided by the chango testing package into a format which will be used by
	// the testrunner module and chango test cmd.
	chango.OnAppStart(func() {
		// Extracting info about available test suites from chango/testing package.
		registeredTests = map[string]int{}
